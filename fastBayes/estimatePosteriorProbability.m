...
%     prItem = sum(bsxfun(@times, exp(logPr_ItemInCategory),  Pr_Category), 2);
%     logPr_Item = log(prItem);
...
% 	end
    for i = 1:Nitems
        Pr_CategoryGivenI(:,i) = logPr_ItemInCategory(i,:) + logPr_Category;% - logPr_Item(i);
    end
end